...
## ⚠️ CRITICAL HARDWARE SAFETY & CLEANING ENGINE
...
A "Files-style" workspace designed for easy navigation, featuring normalized search, bulk deletion, and detailed track information views.
...
* **Smart Vocal Track Swapping:** Align your MIDI using a clean vocal-only stem, and then swap the audio track for a full instrument/drum/bass mix. The system automatically preserves all your breakline alignments. *(Note: Breakline stretching will introduce audio distortions; the severity depends heavily on how closely the lengths of your original MP3 and MIDI match. The closer the starting lengths are, the cleaner the audio will remain).*
...
* **Hybrid Track Integration:** Because playlists were originally built for standard MIDI files, custom compatibility layers were added for hybrid (MIDI + MP3) songs. As long as a hybrid track is marked as **validated**, it can be manually added to playlists, or will automatically populate if its metadata matches the dynamic criteria. Validated hybrid tracks launch utilizing global speaker and sync configurations automatically.
...
### ⚠️ Legacies & Work-in-Progress (Disclaimer)
...
### ⚙️ Windows Automatic Startup (Optional)
